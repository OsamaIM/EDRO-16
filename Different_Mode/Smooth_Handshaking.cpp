...
// ════════════════════════════════════════════════════════════════
//  TUNING ZONE 3  -  GAIT AMPLITUDES
// ════════════════════════════════════════════════════════════════
int HIP_SWING_DEG   = 12;
int KNEE_LIFT_DEG   = 14;
int HIP_ROLL_DEG    = 2;
int ANKLE_COMP_DEG  = 1;

// ════════════════════════════════════════════════════════════════
//  TUNING ZONE 4  -  TIMING
// ════════════════════════════════════════════════════════════════
int PHASE_STEPS  = 12;
int PHASE_SPEED  = 8;
int PHASE_DELAY  = 20;
...
// ════════════════════════════════════════════════════════════════
//  SMOOTH HANDSHAKE  ← FIXED: uses moveParallel, no snap
// ════════════════════════════════════════════════════════════════
void handshake() {
    // 1. Raise and position right arm smoothly
    uint8_t armCh[]  = { CH_R_SHOULDER_PITCH, CH_R_SHOULDER_ROLL, CH_R_ELBOW };
    int     armTgt[] = { 135, 125, 40 };
    moveParallel(armCh, armTgt, 3, 20, 10);
    delay(500);

    // 2. Pump elbow 3 times
    for (int i = 0; i < 3; i++) {
        uint8_t eCh[] = { CH_R_ELBOW };
        int     eDn[] = { 145 };
        int     eUp[] = { 105 };
        moveParallel(eCh, eDn, 1, 8, 8);
        delay(60);
        moveParallel(eCh, eUp, 1, 8, 8);
        delay(60);
    }
    delay(300);

    // 3. Return arm to neutral smoothly
    int retTgt[] = {
        NEUTRAL[CH_R_SHOULDER_PITCH],
        NEUTRAL[CH_R_SHOULDER_ROLL],
        NEUTRAL[CH_R_ELBOW]
    };
    moveParallel(armCh, retTgt, 3, 20, 10);
    delay(600);
}
...
void gaitPhaseB_swingLeft() {
    setServo(CH_L_SHOULDER_PITCH, 75);
    setServo(CH_R_SHOULDER_PITCH, 75);
...
void gaitPhaseD_swingRight() {
    setServo(CH_L_SHOULDER_PITCH, 105);
    setServo(CH_R_SHOULDER_PITCH, 105);
...
void setup() {
    ...
    Serial.println("\n========================================");
    Serial.println("  HANDSHAKE BOT - IR TRIGGER BUILD");
    Serial.println("========================================\n");
    ...
    Serial.println("Ready. Waiting for IR trigger...\n");
}

// ════════════════════════════════════════════════════════════════
//  LOOP  —  IR triggers handshake; guard prevents re-trigger
//           during motion
// ════════════════════════════════════════════════════════════════
bool handshakeRunning = false;   // ← guard flag (NEW)

void loop() {
    bool detected = (digitalRead(IR_PIN) == LOW);

    // Rising edge + debounce + not already running
    if (detected && !lastIR &&
        (millis() - debounceTime > 700) &&
        !handshakeRunning)                   // ← guard check (NEW)
    {
        debounceTime     = millis();
        handshakeRunning = true;             // ← lock (NEW)

        standNeutral();   // clean start position
        delay(200);
        handshake();      // perform smooth handshake
        delay(500);       // brief pause before listening again

        handshakeRunning = false;            // ← unlock (NEW)
    }
