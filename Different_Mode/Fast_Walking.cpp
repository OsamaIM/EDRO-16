/*  ================================================================
    main.cpp  -  Continuous Balanced with fast Bipedal Walking
    ================================================================
    PURPOSE: Robot walks forward continuously  (not one cycle then
             stop) using a 4-phase weight-shift gait for balance.
                                                    
    !! IMPORTANT - YOU MUST TUNE THE SIGN VALUES BELOW !!
    Every robot's servo horns are mounted differently, so "increase
    angle" might mean "leg goes forward" on YOUR left hip but
    "leg goes backward" on your right hip. This code is written
    generically with SIGN multipliers (+1 / -1) for every joint
    so you can flip direction without rewriting the gait logic.

    HOW TO TUNE (do this BEFORE expecting it to walk well):
      1. Upload as-is. Watch one full gait cycle.
      2. If a leg swings the WRONG way (backward instead of
         forward), flip that joint's SIGN from +1 to -1 (or
         vice versa) in the SIGN block below. Re-upload.
      3. If the robot tips over sideways during weight-shift,
         flip the HIP_ROLL or ANKLE sign for that leg.
      4. Repeat until both legs swing forward correctly and
         the robot doesn't tip during weight shift.
      5. Once walking looks right, fine-tune AMPLITUDE constants
         (how many degrees each joint moves) - start small,
         increase gradually.
...
// ════════════════════════════════════════════════════════════════
int HIP_SWING_DEG   = 15;    // how far hip pitches forward/back(nowmal walk 12)
int KNEE_LIFT_DEG   = 14;   // how much knee bends to lift foot
int HIP_ROLL_DEG    = 1;    // lateral weight-shift amount
int ANKLE_COMP_DEG  = 1;    // ankle compensation during weight-shift
...
// ════════════════════════════════════════════════════════════════
int PHASE_STEPS  = 10;   // interpolation steps per phase (smoothness)(normal walk 12)
int PHASE_SPEED  = 6;   // ms delay between each interpolation step(normal walk 8)
int PHASE_DELAY  = 10;   // ms pause after each phase completes(normal walk20)
